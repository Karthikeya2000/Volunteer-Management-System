...
function ProfessorChatApp(): JSX.Element {
  const [contacts, setContacts] = useState();
  const [messages, setMessages] = useState();
  const [message, setMessage] = useState<string>('');
  const [activeUser, setActiveUser] = useState<string>('');

  const { data: graduatesData } = useQuery({
  ...
  // @ts-ignore */}
    return items?.map(item => (
      <div
        className={"message " + (item.other_user == activeUser ? "other" : "me")} >
      ...
      <div className="app">
        ...
        <div className="messages">
          ...
          <form className="messages-inputs" onSubmit={handleSubmit}>
            ...
            <button type="submit"><i className="material-icons" hidden={!activeUser}>send</i></button>
          </form>
        </div>
      </div>
    </div>
...
export default ProfessorChatApp;
